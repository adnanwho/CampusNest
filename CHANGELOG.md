...
- Added shared portal navigation, property cards, detail modals, role switching, and responsive styling.

### Changed

- Hardcoded admin demo credentials updated and verified against backend demo accounts.
- Removed hardcoded JWT secret fallback from `application.yml`; `JWT_SECRET` must be provided via environment variable.
- Updated misleading "AI" labels to honest, human "CampusNest Match" / "CampusNest Verified".
- Frontend availability badges now respect backend `availabilityStatus` field first, with local ratio fallback.
- Frontend consumes backend `matchScore` and `aiExplanation`, with client-side fallback matching.
...
- Property card verification badge changed from "Verified on Blockchain" to "CampusNest Verified".
...
- End-to-end runtime verification completed for all key user flows.
